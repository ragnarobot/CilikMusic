...
from CilikMusic.Utils.tgcallsrun import (
    music,
    convert,
    download,
    clear,
...
from CilikMusic.Utils.database.queue import (
    get_active_chats,
    is_active_chat,
    add_active_chat,
    remove_active_chat,
...
from CilikMusic.Utils.database.onoff import (
    is_on_off,
    add_on,
    add_off,
)
from CilikMusic.Utils.database.chats import (
    get_served_chats,
    is_served_chat,
    add_served_chat,
    get_served_chats,
)
from CilikMusic.Utils.helpers.inline import (
    play_keyboard,
    search_markup,
    play_markup,
    playlist_markup,
...
from CilikMusic.Utils.database.blacklistchat import (
    blacklisted_chats,
    blacklist_chat,
    whitelist_chat,
)
from CilikMusic.Utils.database.gbanned import (
    get_gbans_count,
    is_gbanned_user,
    add_gban_user,
    add_gban_user,
)
from CilikMusic.Utils.database.theme import (
    _get_theme,
    get_theme,
    save_theme,
)
from CilikMusic.Utils.database.assistant import (
    _get_assistant,
    get_assistant,
    save_assistant,
)
...
from CilikMusic.Utils.helpers.greypepek import subcribe
from CilikMusic.Utils.helpers.decorators import authorized_users_only
from CilikMusic.Utils.helpers.decorators import errors
from CilikMusic.Utils.helpers.filters import command
from CilikMusic.Utils.helpers.gets import (
    get_url,
    themes,
    random_assistant,
    ass_det,
)
from CilikMusic.Utils.helpers.logger import LOG_CHAT
from CilikMusic.Utils.helpers.thumbnails import gen_thumb
from CilikMusic.Utils.helpers.chattitle import CHAT_TITLE
from CilikMusic.Utils.helpers.ytdl import ytdl_opts 
from CilikMusic.Utils.helpers.inline import (
    play_keyboard,
    search_markup2,
    search_markup,
...
@Client.on_message(command(["play", f"play@Tg_Vc_00_Bot", "p"]))
@subcribe
...
"""
    ...
    try:
        ...
        if b.status == "kicked":
            await message.reply_text(f"{ASSNAME}(@{ASSUSERNAME}) dibanned di obrolan Anda **{chat_title}**\n\nUnban terlebih dahulu untuk menggunakan Music")
            return
    ...
    if audio:
        ...
        mystic = await message.reply_text(f"**💸 Lagu di Proses....**")
        if audio.file_size > 157286400:
            await mystic.edit_text("Ukuran File Audio Harus Kurang dari 150 mb") 
            return
        duration = round(audio.duration / 60)
        if duration > DURATION_LIMIT:
            return await mystic.edit_text(f"**__Durasi Error__**\n\n**Durasi Yang di izinkan: **{DURATION_LIMIT} minute(s)\n**Durasi yang diterima:** {duration} minute(s)")
        ...
        link = "https://t.me/CilikProject"
        thumb = "Assets/Audio.jpeg"
        videoid = "smex1"
    elif url:
        what = "URL Searched"
        await LOG_CHAT(message, what)
        ...
        if smex > DURATION_LIMIT:
            return await mystic.edit_text(f"**__Durasi Error__**\n\n**Durasi yang di izinkan: **90 minute(s)\n**Durasi diterima:** {duration} minute(s)")
        if duration == "None":
            return await mystic.edit_text("Maaf! Video tidak di dukung")
        if views == "None":
            return await mystic.edit_text("Maaf! Video tidak di dukung")
    ...
    if await is_active_chat(chat_id):
        ...
        await message.reply_photo(
            photo=thumb,
            caption=(f"💡 **Trek ditambahkan ke antrian »** `{position}`\n\n🏷️<b> **Nama:** </b>[{title[:25]}]({url}) \n⏳🕰️<b> **Durasi:** </b> {duration} \n🎧<b> **Requested By:** </b>{checking}"),
            reply_markup=InlineKeyboardMarkup(buttons)
        )
        ...
        photo=thumb,
        reply_markup=InlineKeyboardMarkup(buttons),    
        caption=(f"🏷️<b> **Nama:** </b>[{title[:25]}]({url}) \n🕰️<b> **Durasi:** </b> {duration} \n🎧<b> **Requested by:** </b>{checking}")
    )   
        return await mystic.delete()
...
async def startyuplay(_,CallbackQuery): 
    ...
    smex = int(time_to_seconds(duration))
    if smex > DURATION_LIMIT:
        await CallbackQuery.message.reply_text(f"**__Durasi Error__**\n\n**Durai Yang di izinkan: **90 minute(s)\n**Durasi yang di Terima:** {duration} minute(s)")
        return 
    try:
    ...
    if await is_active_chat(chat_id):
        ...
        photo=thumb,
        caption=(f"💡 **Track di tambahkan di Antrian »** `{position}`\n\n🏷️<b> **Title:** </b>[{title[:25]}]({url}) \n🕰️<b> **Durasi:** </b> {duration} \n🎧<b> **Requested by:** </b>{checking}"),
        reply_markup=InlineKeyboardMarkup(buttons)
    )
        os.remove(thumb)
        await CallbackQuery.message.delete()       
